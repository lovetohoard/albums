...
video_track = Track(
    ...
    tag={
        BasicTag.ARTIST: "V",
        BasicTag.TITLE: "VV",
        BasicTag.ALBUM: "video",
        BasicTag.ALBUMARTIST: "bar",
...
album = Album(path="baz" + os.sep, tracks=[track, video_track])
...
class TestMp4:
    ...
    def test_read_write_m4a_tags(self):
        with TestMp4.tagger.open(track.filename) as file:
            scan = file.scan()
    ...
    def test_read_write_mp4_tags(self):
        with TestMp4.tagger.open(video_track.filename) as file:
            scan = file.scan()
        ...
        assert scan.pictures[0].picture_info.width == scan.pictures[0].picture_info.height == 400
        tags = dict(scan.tags)
        track_tags = video_track.tag_dict()
        assert tags[BasicTag.ARTIST] == tuple(track_tags[BasicTag.ARTIST])
        assert tags[BasicTag.ALBUMARTIST] == tuple(track_tags[BasicTag.ALBUMARTIST])
        assert tags[BasicTag.ALBUM] == tuple(track_tags[BasicTag.ALBUM])
        assert tags[BasicTag.TITLE] == tuple(track_tags[BasicTag.TITLE])
    ...
    def test_detect_mp4_video(self):
        with TestMp4.tagger.open(video_track.filename) as video_file:
            assert video_file.has_video()
        with TestMp4.tagger.open(track.filename) as audio_file:
            assert not audio_file.has_video()

    def test_update_mp4_tags(self):
        TestMp4.tagger.set_basic_tags(
            TestMp4.library / album.path / track.filename,
